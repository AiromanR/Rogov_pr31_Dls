...
public:
    ...
    void triggerEvent(int count) {
        int currentCount = 0;
        for (Node* i = head; (i != nullptr) && (currentCount < count); i = i->next, currentCount++) {
            if (i->messages) {
                i->Handler.messageHandler(i->messages);
            }
            else {
...
int main()
{
    ...
    system.registerHandler(&onError, &error2[0]);
    ...
    system.triggerEvent(6);
    std::cout << std::endl;

    std::cout << "Введите количество уведомлений: ";
    int num;
    std::cin >> num;
    system.triggerEvent(num);
}
